...
ROLE_CASES = [
    ...
    (
        ...
        facts(latest_run_id="run_1", latest_run_passed={"cross_company_isolation": False},
              pending_check_ids=["cross_company_isolation"]),
        ("technical", "run_follow_up", ""),
    ),
    (
        "the product manager interjects once after a passing isolation run",
        state(stage="investigation", pending_run_ids=["run_1"]),
        facts(latest_run_id="run_1", latest_run_passed=PASSING_ISOLATION,
              pending_check_ids=list(PASSING_ISOLATION)),
    ...
    (
        ...
        facts(latest_run_id="run_2", latest_run_passed=PASSING_ISOLATION,
              pending_check_ids=list(PASSING_ISOLATION)),
        ("technical", "run_follow_up", ""),
    ),
    (
        "an older pass does not earn the product manager's interjection",
        state(stage="investigation", pending_run_ids=["run_1", "run_2"]),
        facts(latest_run_id="run_1", latest_run_passed=PASSING_ISOLATION,
              pending_check_ids=["cross_company_isolation"]),
        ("technical", "run_follow_up", ""),
    ),
    ...
    (
        ...
        facts(pending_check_ids=["revocation_next_request"]),
        ("customer", "run_follow_up", ""),
    ),
    (
        "a pending revocation run that timed out is still the customer's",
        state(stage="changed_condition", revocation_introduced=True, pending_run_ids=["run_3"]),
        facts(latest_run_id="run_0", latest_run_passed={"cross_company_isolation": True},
              pending_check_ids=["revocation_next_request"]),
        ("customer", "run_follow_up", ""),
    ),
    (
    ...
    (
        ...
        facts(latest_run_id="run_5", latest_run_passed={"revocation_next_request": True},
              pending_check_ids=["cross_company_isolation"]),
        ("technical", "run_follow_up", ""),
    ),
    (
