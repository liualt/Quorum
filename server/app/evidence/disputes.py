...
review until a reviewer resolves the dispute (PRD sections 5 and 12). A replay
whose results differ marks the findings that cite the original run the same
way, under its own reason, and both results are kept.
"""
...
def create_dispute(conn, bus, interview_id: str, segment_id: str, proposed_text: str, reason: str):
    ...
    review_reason = dispute_reason(row["id"])
    ...
    row = repo.update_dispute(conn, row["id"], affected_finding_ids_json=json.dumps(affected))
